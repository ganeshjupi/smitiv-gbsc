...
class Testing_vendor_bill_payment extends React.Component {
    constructor(props) {
        super(props);
        ...
        this.state = {
            ...
            edit_customer_receive_payment: JSON.parse(localStorage.getItem("edit_customer_receive_payment")),
            dropdown: "",
            rec_acc: '',
            inbox_list: [],
            response_stus: 0,
                ...
                fetch(
                    ...
                    .then((data) => {
                        if (this.state.clientHomeCurrency != '' && this.state.clientHomeCurrency != undefined) {
                            this.setState({
                                exchange_rate: (data.rates[this.state.clientHomeCurrency]).toFixed(4),
                                exchangeRate: (data.rates[this.state.clientHomeCurrency]).toFixed(4),
                                exchange_rate_temp: (data.rates[this.state.clientHomeCurrency]).toFixed(4)
                            })
                        }
                    })
            } else {
    ...
    handleChange = () => {
        ...
        this.setState({
            ...
            this.customer_invoicelist(variable_pay_type);
            ...
            if (response.status === 1) {
                ...
                if (this.props.location.state !== '' && this.props.location.state !== null && this.props.location.state !== undefined) {
                    response.list.forEach((item, i) => {
                        if (item.name == "Accounts Payable" + "-" + this.props.location.state[1]) {
                           // alert('ji')
                            this.setState({
    ...
    customer_recived_payment = (x) => {
        ...
        if (this.state.isEdit) {
            payment_date = moment(jQuery('#date').val()).format("YYYY-MM-DD")
        } else {
        ...
        FetchAllApi.testing_vendor_bill_payment(
            customer_id,
            payor_name,
            client_id,
            descripation,
            ...
            (err, response) => {
                ...
                if (response.status === 1) {
                    ...
                    if (x === "save") {
                        ...
                        this.props.history.push("/loading", ["/testing_Customer_receive_payment"]);
                    } else {
                        alert("Success ! succesfully saved");
...
export default Testing_vendor_bill_payment;
