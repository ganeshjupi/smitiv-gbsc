...
class create_salesorder extends React.Component {
  constructor(props) {
    super(props)
  ...
  handleChangeItems = async (e, itemid) => {
    ...
    for (var i = itemid; i >= 0; i--) {
      ...
      if (!this.state.isChecked) {
        ...
        var item_list = {
          list: jQuery('#cust_items' + i).val(),
          itemids: jQuery('#itemsid' + i).val() ? jQuery('#itemsid' + i).val() : 0,
          ...
          tax_id: jQuery('#selectednow_id' + i).text(),
  ...
  convertHtmlToPdf(e) {
    if (this.state.selected_templateName === 'Choose') {
      ...
      if (
        ...
        let shipping_address = jQuery('#invoice_to').val()
  ...
  save_template = (e, btn_type) => {
    ...
    if (
      ...
      let data = {
        ...
        salesorder_date: invoice_date,
        company_name: company_name,
        type: type,
        salesorder_number: invoice_number,
  ...
  render() {
    console.log('hhhhh', this.state.myarray)
    var data = ''
                ...
                <div>
                  <div className='content-top col-md-12 col-xs-12'>
                    ...
                    >
                              ...
                              <a href='javascript:;' onClick={
                                () => {
                                  let data = {
                                    ...
                                    selected_templateName: this.state.selected_templateName,
                                    selectCustomer: this.state.selectCustomer,
                                    vendorName: jQuery('#variable_pay_type').val(),
                                    date: jQuery('#invoice_date').val(),
                                    salesno: jQuery('#invoice_no').val(),
                                    jobid: this.state.job_id,
                                    salesorderto: jQuery('#invoice_to').val(),
                                    memo: jQuery('#footer_memo').val(),
                                    thanking_message: jQuery('#footeroptionthanksmessages').val(),
                                    terms_and_conditions: jQuery('#termcondmsg').val(),
                                    amountinwords: jQuery('#amount_in_words').val(),
                                    foreignsubtotal: jQuery('#homecurrencytyt').text(),
                                    homesubtotal: jQuery('#foreigncurrencytyt').text(),
                                    fortax: jQuery('#home_tax_total').text(),
                                    homecurenncytax: jQuery('#foreign_tax_total').text(),
                                    forgingrandtotal: jQuery('#home_grand_total').text(),
                                    homegrandtotal: jQuery('#foreign_grand_total').text(),
                                  ...
                                  // }))
                                  this.props.history.push({ pathname: '/create_invoice', state: { invoicessalesorder: data } })//pass data from salesorder data to invoice using invoicessalesorder using props
                                }
                                // (e)=>this.props.history.push(
                                //   '/create_salesorder',
                                //   [this.collectdata()]
                                // )
                              }>Create invoice</a>
                              ...
                              <a href='javascript:;'
                                onClick={
                                  () => {
                                    ...
                                    // }))
                                    this.props.history.push({ pathname: '/create_purchaseorder', state: { purchaseorder: data } }) // pass data from salesorder data to purchase order using purchaseorder using props
                                  }
                                  // (e)=>this.props.history.push(
                                  //   '/create_salesorder',
                                  //   [this.collectdata()]
                      ...
                      <div
                        className='form-group mar-rgt '
                        style={{ marginLeft: -2, width: 114, height: 69 }}
                      >
...
export default create_salesorder
