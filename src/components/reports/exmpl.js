...
class open_invoice extends React.Component {
  ...
  recursiveCategeory = (subcat) => {
    ...
    const swingHead = (subcat) => {
      if (subcat != undefined) {
        ...
        if (Number(blockMe) == 1) {
          let itemsPrimary = Object.values(subcat[0]);
          if (itemsPrimary != undefined) {
    ...
    swingHead(useFirst);
    console.log("jjdjdjdj", result);
    return this.renderRows(result);
  };
  ...
  callAPIDATA() {
    ...
    FetchAllApi.filter_column(
      this.state.all_report_name_id,this.state.logged_client_id,null,
                          ...
                          <tbody>
                            ...
                            {this.state.response &&
                              this.state.response.map((entry, key) => {
                                var myarray = this.recursiveCategeory(
                                  entry.sub_categories
                                );
                                var chkme = Object.values(
                                  entry.sub_categories[0]
                                );
                                console.log("uuuuu", myarray);
                                if (
                                  myarray.length > 0 ||
                                  ...
                                  return (
                                    <React.Fragment>
                                      ...
                                      {myarray}
                                      {entry.invoices.length > 0 &&
                                        this.renderRows(entry.invoices)}
                                    </React.Fragment>
                                  );
                                }
                              })}
